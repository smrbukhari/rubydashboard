BiSquare.BiDashboardHelper = {
  staticPlotsInitialize: function() {
    $.ajax({
      ...
      success: function (response) {
        BiSquare.FiltersHelper.populateFilters(['filter1', 'filter2', 'filter3'], response.data);
  ...
  staticPlotGeneration: function(settings) {
    var filter1 = $('#' + settings.filter1Id), filter2 = $('#' + settings.filter2Id), filter3 = $('#' + settings.filter3Id),
